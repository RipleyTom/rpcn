...
impl Client {
	...
	pub async fn process(&mut self) {
		if *self.terminate_watch.recv.borrow_and_update() == true {
			return;
		}
			...
			if !self.authentified {
				...
				tokio::select! {
					_ = self.terminate_watch.recv.changed() => {
						assert_eq!(*self.terminate_watch.recv.borrow(), true);
	...
	async fn process_command(&mut self, command: u16, data: &mut StreamExtractor, reply: &mut Vec<u8>) -> Result<(), ()> {
		...
		match command {
			...
			CommandType::RetrieveScores => self.retrieve_scores(data, reply).await,
	...
	fn get_database_connection(&mut self, reply: &mut Vec<u8>) -> Result<r2d2::PooledConnection<r2d2_sqlite::SqliteConnectionManager>, ()> {
